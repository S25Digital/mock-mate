...
export class MockMate {
  ...
  private initializeMockEndpoints(): void {
    ...
    Object.keys(paths).forEach((path) => {
      ...
      Object.keys(methods).forEach((method) => {
        // Cast method to a valid HTTP method
  ...
  private setupEndpoint(path: string, method: keyof OpenAPIV3.PathItemObject): void {
    ...
    this.app[method.toLowerCase() as keyof Application](path, (req: Request, res: Response) => {
      const response = this.dynamicConfig[endpointKey] || this.defaultConfig[endpointKey];
      res.status(response.statusCode || 200).json(response.body);
    });
  }

  // Generate default response based on OpenAPI spec
  private generateDefaultResponse(path: string, method: keyof OpenAPIV3.PathItemObject): { statusCode: number, body: any } {
  ...
  public updateMockConfig(path: string, method: keyof OpenAPIV3.PathItemObject, statusCode: number, responseBody: any): void {
    const endpointKey = `${method.toUpperCase()} ${path}`;
